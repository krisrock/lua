...
local master_switch = ui_new_checkbox("RAGE", "Aimbot", "Enable \aFFF97DFFAdaptive\a Weapon")
...
local state_indicator = ui.new_multiselect("Rage", "other", "Weapon Indicators", {"Text Value", "Text Value[Crosshair]", "Damage Value", "Damage Value[Crosshair]", "Hitchance Value", "Hitchance Value[Crosshair]"})
...
client.set_event_callback("paint", function(e)
    ...
    if ui_get(ovr_head) then
        renderer_indicator(r, g, b, a, "Force Head")
	end
 ...
 --[[    if ui.get(damage_ove_key) or ui.get(damage_ove_key2) then
	    renderer.text(screen_x / 2 + 12, screen_y / 2 - 15,r, g, b, a, "c", 0, "" .. (ui.get(ref_damage) <= 100 and ui.get(ref_damage) or ("HP+" .. (ui.get(ref_damage) - 100))))
	end ]]
end)
