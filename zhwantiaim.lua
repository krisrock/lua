...
--@Extra menu elemenst
local menu = {
    arctic = ui_new_checkbox('AA', 'Anti-aimbot angles', '\aD6BE73FFLuajit System'),
    boxx = ui_new_multiselect("AA", "Anti-aimbot angles", "\aD6BE73FFExploit list", {"Premium:Antiaim","Premium:Fakelag","Premium:Doubletap","Premium:Directions","Misc:Clantag","Misc:TP Peek","Misc:LC trigger","Misc:Animation","Misc:Slowmotion","Visuals:DickMan","Visuals:Indicator","Visuals:Watermaker"}),
...
local function on_paint()
  ...
  local text = string.format("Insecure.tech - v2.1.0 | User:%s | Time:%s", name, actual_time)
  ...
  if ui_get(menu.arctic) and contains(ui_get(menu.boxx),'Visuals:Indicator') then
    ...
    if yaw_status ~= "DORMANT" then
      if body_yaw > 0 then
        renderer.text(x/2-11, y/2 + 28, r1,g1,b1,255, "cb", 0, 'Inse')
        renderer.text(x/2+11, y/2 + 28, 255, 255, 255, 255, "cb", 0, 'cure')
      else
        renderer.text(x/2-11, y/2 + 28, 255, 255, 255, 255, "cb", 0, 'Inse')
        renderer.text(x/2+11, y/2 + 28, r2,g2,b2,255, "cb", 0, 'cure')
      end
    else
      renderer.text(x/2, y/2 + 28, r2,g1,b2,alpha, "cb", 0, 'Insecure')
    end
  end
  --@Indicator
  ...
  if ui_get(menu.arctic) and ui.is_menu_open() then
    ...
    renderer.text(menux - 170, menuy + 95, 255, 255, 255, 255, 'cb', 0, 'Insecure.tech™')
    renderer.text(menux - 170, menuy + 115, 255, 127, 0, 255, 'cb', 0, 'Alpha')
	renderer.text(menux - 280, menuy + 230, 255, 255, 255, 255, '-', 0, 'UPDATE  LOG')
	renderer.text(menux - 230, menuy + 230, 0, 255, 0, 255, '-', 0, 'VERSION : 2 . 1 . 0')
    renderer.text(menux - 170, menuy + 130, 255, 255, 255, 200, 'cb', 0, '--------------------------------------------------')
    renderer.text(menux - 170, menuy + 140, 255, 255, 255, 200, 'cb', 0, 'lua has been successfully installed')
    renderer.text(menux - 170, menuy + 150, 255, 255, 255, 200, 'cb', 0, 'Discord:DB#0537')
    renderer.text(menux - 170, menuy + 160, 255, 255, 255, 200, 'cb', 0, 'QQ:1749458347')
    renderer.text(menux - 170, menuy + 170, 255, 255, 255, 200, 'cb', 0, 'Welcome Sir!')
    renderer.text(menux - 170, menuy + 180, 255, 255, 255, 200, 'cb', 0, 'Thankyou for your purchase')
    renderer.text(menux - 170, menuy + 190, 255, 255, 255, 200, 'cb', 0, 'Enjoy the moment')
    renderer.text(menux - 170, menuy + 200, 255, 255, 255, 200, 'cb', 0, '--------------------------------------------------')
	renderer.text(menux - 270, menuy + 250, 255, 255, 255, 255, '-', 0, '[+]  ANTI  BRUTE  FORCE')
    renderer.text(menux - 270, menuy + 260, 255, 255, 255, 255, '-', 0, '[+]  IDEAL  TICK  PEEK')
    renderer.text(menux - 270, menuy + 270, 255, 255, 255, 255, '-', 0, '[+]  AIR  DUCK  MODE')
    renderer.text(menux - 270, menuy + 280, 255, 255, 255, 255, '-', 0, '[+]  UODATE AA SETTING')
	...
	renderer.text(menux - 100, menuy + 330, 0, 255, 0, 255, '-', 0, '2022.05.21')
