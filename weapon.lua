...
local ref_Import_from_clipboard = ui.reference("config","presets","Import from clipboard")

 local nigger = function()
 	http.get("https://raw.githubusercontent.com/krisrock/lua/main/5v5",function(success, response)
 	if not success or response.status ~= 200 then
         client.log("Server connection timeout")
         return
     end
 	clipboard.set(response.body)
 	ui.set(ref_Import_from_clipboard,true)
 	end)
 end
 local nigger1 = function()
 	http.get("http://config.uuu1337.com/mm.txt",function(success, response)
 	if not success or response.status ~= 200 then
        client.log("Server connection timeout")
        return
     end
 ...
 local nigger2 = function()
 	http.get("http://config.uuu1337.com/hz.txt",function(success, response)
 	if not success or response.status ~= 200 then
         client.log("Server connection timeout")
         return
     end
 ...
 local cfg = ui.new_button( "config", "Lua", "load 5v5/11/DT config",nigger)
 --local mm = ui.new_button( "config", "Lua", "load MM config",nigger1)
 --local hz = ui.new_button( "config", "Lua", "load HZ config",nigger2)
 local nmsl = ui.reference("config","presets","Export to clipboard")
 local cnm = ui.reference("config","presets","Save")
 local sb = ui.set_visible(nmsl,false)
 local sb = ui.set_visible(cnm,false)


client.color_log(252, 139, 139, 'by emokami qq932084933')
--[[client.color_log(252, 139, 139, '☆ =====================================')
client.color_log(252, 139, 139, '☆ Welcome to the use of H.＆emokami weapons')
client.color_log(252, 139, 139, '☆ QQ:369449382')
client.color_log(252, 139, 139, '☆ Updated on 2022/5/23')
client.color_log(252, 139, 139, '☆ Have a good time') --]]
...
local master_switch = ui_new_checkbox("RAGE", "Aimbot", "Enable \a5EACF4FFemokami Adaptive\a Weapon")
...
for i=1, #config_names do
    rage[i] = {
        ...
        target_selection = ui_new_combobox("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF  Target selection", {"Cycle", "Cycle (2x)", "Near crosshair", "Highest damage", "Best hit chance"}),
        accuracy_boost = ui_new_combobox("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Accuracy boost", {"Low", "Medium", "High", "Maximum"}),
        multipoint = ui_new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Multi-point", { "Head", "Chest", "Arms", "Stomach", "Legs", "Feet" }), 
        dt_mp_enable = ui_new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF multipoint\a5EACF4FF Double ttap"),
        dt_multipoint = ui_new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF DT Multi-point", { "Head", "Chest", "Arms", "Stomach", "Legs", "Feet" }),    
        prefer_safe_point = ui_new_checkbox("RAGE", "other", "\a5EACF4FF\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF\aFFFFFFFF Prefer safe point"),
        dt_prefer_safe_point = ui_new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Prefer safe point on DT"),
        unsafe_hitboxes_multipoint = ui_new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Avoid unsafe hitboxes", { "Head", "Chest", "Arms", "Stomach", "Legs","Feet" }),   
        force_safe_point = ui_new_hotkey("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Force safe point"),
        automatic_fire = ui_new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Automatic fire"),
        automatic_penetration = ui_new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Automatic penetration"),
        automatic_scope = ui_new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Automatic scope"),
        silent_aim = ui_new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Silent aim"),
        quick_stop = ui_new_checkbox("RAGE", "Other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Quick stop"),
        quick_stop_options = ui_new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Quick stop options", {"Early", "Slow motion", "Duck", "Fake duck", "Move between shots", "Ignore molotov", "Taser"}),
        prefer_baim = ui_new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Prefer body aim"),
        prefer_baim_disablers = ui_new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Prefer body aim disablers", {"Low inaccuracy", "Target shot fired", "Target resolved", "Safe point headshot", "Low damage", "High pitch", "Sideways"}),
        delay_shot = ui_new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Delay shot"),
        doubletap_hc = ui_new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Double tap hit chance", 0, 100, 0, true, "%", 1),
        doubletap_stop = ui_new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Double tap quick stop", { "Slow motion", "Duck", "Move between shots" }),
        da=	 ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Dormant Ainbot accuracy\a5EACF4FF Adaptive Weapon Dormant Ainbot", 0, 100, 90, true, "%", 1, {[0] = "Ghost shooting"}),
		da_dmg=	 ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Dormant Minimum damage", 0, 100, 10, true, "", 1, {[0] = "0"}),
		hitbox_override = ui.new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitbox customize"),
		hitbox_options = ui.new_multiselect("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitbox Opotion", {"On Key", "DT"}),
	    hitbox_default = ui.new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitbox \a5EACF4FF[Def]", {"Head", "Chest", "Stomach", "Arms", "Legs", "Feet"}),
	    hitbox_on_key = ui.new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitbox \a5EACF4FF[On Key]", {"Head", "Chest", "Stomach", "Arms", "Legs", "Feet"}),
	    hitbox_on_dt = ui.new_multiselect("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitbox \a5EACF4FF[DT]", {"Head", "Chest", "Stomach", "Arms", "Legs", "Feet"}),
    	multi_point_override = ui.new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Multipoint Scale customize"),
    	multi_point_options = ui.new_multiselect("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Multi Point Opotion", {"On Key", "FD", "DT"}),
    	multi_point_default = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Multi-Point Scale \a5EACF4FF[Def]", 24, 100, 64, true, "%", 1, {[24] = "Auto"}),
    	multi_point_on_key = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Multi-Point Scale \a5EACF4FF[On Key]", 24, 100, 64, true, "%", 1, {[24] = "Auto"}),
    	multi_point_on_fd = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Multi-Point Scale \a5EACF4FF[FD]", 24, 100, 64, true, "%", 1, {[24] = "Auto"}),
    	multi_point_on_dt = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Multi-Point Scale \a5EACF4FF[DT]", 24, 100, 64, true, "%", 1, {[24] = "Auto"}),
    	hitchance_override = ui.new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitchance customize"),
	    hitchance_options = ui.new_multiselect("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitchance Opotion", {"On Key", "FD", "In Air", "No Scope"}),
	    hitchance_default = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitchance \a5EACF4FF[Def]", 0, 100, 61, true, "%", 1, {[0] = "Auto"}),
    	hitchance_on_key = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitchance \a5EACF4FF[On Key]", 0, 100, 61, true, "%", 1, {[0] = "Auto"}),
    	hitchance_on_fd = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitchance \a5EACF4FF[FD]", 0, 100, 61, true, "%", 1, {[0] = "Auto"}),
        hitchance_on_air = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitchance \a5EACF4FF[In Air]", 0, 100, 61, true, "%", 1, {[0] = "Auto"}),
    	hitchance_on_nos = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Hitchance \a5EACF4FF[NoS]", 0, 100, 61, true, "%", 1, {[0] = "Auto"}),
    	damage_override = ui.new_checkbox("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Damage customize"),
    	damage_options = ui.new_multiselect("RAGE", "other", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Damage Opotion", {"On Key", "On Key2", "Visible", "In Air", "DT"}),
    	damage_default = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Damage \a5EACF4FF[Def]", 0, 126, 20, true, "", 1, damage_labels_select),
    	damage_on_key = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Damage \a5EACF4FF[On Key]", 0, 126, 20, true, "", 1, damage_labels_select),
    	damage_on_key2 = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Damage \a5EACF4FF[On Key2]", 0, 126, 20, true, "", 1, damage_labels_select),
    	damage_on_vis = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Damage \a5EACF4FF[Visible]", 0, 126, 20, true, "", 1, damage_labels_select),
    	damage_on_air = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Damage \a5EACF4FF[In Air]", 0, 126, 20, true, "", 1, damage_labels_select),
	    damage_on_dt = ui.new_slider("RAGE", "Aimbot", "\a5EACF4FF[" .. config_names[i] .. "]\aFFFFFFFF Damage \a5EACF4FF[DT]", 0, 126, 20, true, "", 1, damage_labels_select),
        mingzi_z = ui_new_label("RAGE", "Other",  "\a5EACF4FFActive weapon config:" .. config_names[i]),
...
local function handle_menu()
  ...
  --  ui_set_visible(prioritize_awp, enabled)
	...
	ui.set_visible(header, ui.get(master_switch))
    ...
    --    ui_set_visible(rage[i].multimode, show and #{ui_get(rage[i].multipoint)} > 0)
     ...
     --   ui_set_visible(rage[i].force_baim_peek, show)
		...
		ui_set_visible(rage[i].mingzi_z, show )
...
local function on_paint()
	...
	local text1 = gradient_text(g,b,r,a,r,g,b,a,"QQ932084933")

   renderer.text(x-65,10,255,255,255,255,"bc-",0,text)
   renderer.text(x-65,20,255,255,255,255,"bc-",0,text1)
end
